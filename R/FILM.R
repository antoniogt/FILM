...
#'
#' @examples
FILM <- function(dataset, formula, df_aux=6, IAAs=c("IPIP","SMOTE","ROSE"),models=c("ranger","glm"), hyperparameters=NULL, metrics=c("ACCURACY","KAPPA","F1"), cv=5, prop_min =0.05, prop_max = 0.4, tC=trainControl( summaryFunction = FILM::metrics,  allowParallel = TRUE,  classProbs = TRUE),metric_max="KAPPA",num.trees=200,return.uic=T,random_seed=random_seed){
  ...
  for(df in 1:length(subdatasets)){
    ...
    for(folds in 1:cv){
      ...
      val_split <- train_set[-train_index, ]
  ...
  for(v in 1:(length(models)*length(IAAs))){
    ...
    list_metrics_total<-append(list_metrics_total,FILM::metrics_dfs(ml=models_trained[as.double(v+((length(models)*length(IAAs)))*(0:(cv-1)))], formula =formula,model_names=unique(names_models[as.double(v+((length(models)*length(IAAs)))*(0:(cv-1)))]), val=datasets_test,metrics=metrics)) #Sirve para hacer plots Jose Adrian
